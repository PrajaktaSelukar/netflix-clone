const apiKey = '838daaf96011743140614bfeeb387e4c';
...
function fetchTrendingMovies() {
    fetchAndBuildSection(apiPaths.fetchTrending, 'Trending')
    .then(list => {
...
function buildBannerSection(movie){
    ...
    bannerCont.style.backgroundImage = `url(${img_path}${movie.poster_path})`;
...
function buildMoviesSection(list, categoryName) {
    const moviesCount = document.getElementById('movies-cont');
    const moviesListHTML = list.map(item => {
        return `
            <img class="movie-item" src="${img_path}${item.poster_path}" alt="${item.title}">
        `;
    }).join('');
    
    const moviesSectionHTML = `
        <h1 class="movie-section-heading">${categoryName} <span class="explore-nudge">Explore All</span></h1>
